...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // If no vessels loaded from localStorage, try automatic DB connection
    if (app.vessels.length === 0) {
...
const app = {
    ...
    clockCities: ["Panama", "Altamira", "Salvador", "Antwerp"],
    vessels: [],
    filteredVessels: [],
    clockIntervals: []
};

// Save vessels to localStorage as backup
...
// Try to connect to database with default settings
function connectToDefaultDatabase() {
    document.getElementById('dbHost').value = app.dbConfig.host;
    document.getElementById('dbPort').value = app.dbConfig.port;
    document.getElementById('dbName').value = app.dbConfig.database;
    document.getElementById('dbUser').value = app.dbConfig.user;
    document.getElementById('dbPassword').value = app.dbConfig.password;
    
    const statusElement = document.getElementById('dbStatus');
    if (statusElement) {
        statusElement.textContent = "Attempting automatic connection...";
    }
    
    // Use the API proxy to connect to database
    fetch(`${app.apiBaseUrl}/connect`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify(app.dbConfig)
    })
    ...
    .then(data => {
        if (data.success) {
            ...
            console.error('Auto-connection failed:', data.message);
            // Fall back to localStorage if it exists
            loadVesselsFromLocalStorage();
        }
    })
    .catch(error => {
...
function saveEditedCell(form) {
    ...
    if (app.dbConnection) {
        const vesselId = app.vessels[index].id;
        updateVesselInDatabase(vesselId, columnName, newValue);
    } else {
    ...
    if (confirm(`Change inspection timing for ${vessel.vessels} from ${currentTiming} to ${newTiming}?`)) {
        ...
        if (index !== -1) {
            ...
            if (app.dbConnection && app.vessels[index].id) {
                updateVesselInDatabase(app.vessels[index].id, 'inspectionTiming', newTiming);
            } else {
                saveVesselsToLocalStorage();
            }
...
function markAsDeparted() {
    ...
    if (app.dbConnection && app.vessels[app.selectedRow].id) {
        updateVesselInDatabase(app.vessels[app.selectedRow].id, 'departed', !currentStatus);
    } else {
        saveVesselsToLocalStorage();
    }
...
function connectToDatabase(form) {
    ...
    // Make API call to connect to database through proxy
    fetch(`${app.apiBaseUrl}/connect`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
...
function fetchVesselsFromDatabase() {
    fetch(`${app.apiBaseUrl}/vessels?config=${encodeURIComponent(JSON.stringify(app.dbConfig))}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                app.vessels = data.vessels;
                populateMainTable();
                updateDepartureTable();
...
// Function to update a vessel in the database
function updateVesselInDatabase(id, field, value) {
    const updateData = {
        id: id,
        field: field,
        value: value
    ...
    fetch(`${app.apiBaseUrl}/vessels/${id}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    .then(data => {
        if (data.success) {
            ...
            console.error('Error updating vessel:', data.message);
        }
...
// Calculate time left based on ETD and port timezone
function calculateTimeLeft(etdStr, port) {
    try {
        // Get timezone for port
        const timezoneName = app.portToTimezone[port] || "America/Panama";
        
        // Current time in port's timezone
        const now = new Date();
        
        // Parse the ETD string (DD/MM - HHMM)
        const [datePart, timePart] = etdStr.split(' - ');
        ...
        etdDate.setDate(parseInt(day));
        etdDate.setMonth(parseInt(month) - 1);  // JavaScript months are 0-indexed
        etdDate.setHours(parseInt(hour), parseInt(minute), 0, 0);
        
        // Calculate time difference in seconds
        const diffInSeconds = (etdDate - now) / 1000;
...
function informUWI() {
    ...
    if (app.filteredVessels.length === a) { 
